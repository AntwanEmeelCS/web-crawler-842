...
let btnTryAnotherRecipe = document.getElementById("btnTryAnotherRecipe");

function FillRecipeImage(recipeObject) {
  let imageDiv = document.getElementById("recipeImage");
  imageDiv.src = recipeObject.imgCover;
  imageDiv.alt = recipeObject.image;
}

function FillRecipeRating(recipeObject) {
  let divRecipeRating = document.querySelector(".recipeRating");
  divRecipeRating.innerHTML = `<div
              class="ratingContent d-flex flex-row justify-content-around align-content-center"
            >
              <i class="fa-solid fa-star fs-5 p-1"></i>
              <span class="fw-semibold fs-6 p-1">${recipeObject.ratingsAverage}</span>
              <span class="text-secondary fs-6 p-1">(${recipeObject.ratingsQuantity} reviews)</span>
            </div>`;
}

function FillRecipeCookingTime(recipeObject) {
  let divCookingTime = document.querySelector(".recipeCookingTime");
  divCookingTime.innerHTML = `<div class="d-flex flex-row justify-content-evenly mx-4 p-4">
              <div
                class="d-flex flex-column align-items-center justify-content-center"
              >
                <i class="fa-solid fa-clock fs-3" style="color: orange"></i>
                <span class="text-secondary fs-6">Prep Time </span>
                <span class="text-black fw-bold fs-6">${recipeObject.prepTime}</span>
              </div>
              <div
                class="d-flex flex-column align-items-center justify-content-center"
              >
                <i class="fa-solid fa-fire-burner fs-3"></i>
                <span class="text-secondary fs-6" style="color: red"
                  >Cook Time
                </span>
                <span class="text-black fw-bold fs-6">${recipeObject.cookTime}</span>
              </div>
              <div
                class="d-flex flex-column align-items-center justify-content-center"
              >
                <i class="fa-solid fa-users fs-3" style="color: blue"></i>
                <span class="text-secondary fs-6">Servings</span>
                <span class="text-black fw-bold fs-6">${recipeObject.servings}</span>
              </div>
            </div>
          </div>`;
}

function FillRecipeTitle(recipeObject) {
  let divTitle = document.querySelector(".recipeTitle");
  let cartoona = `<div class="recipeTags d-flex flex-row align-items-start">
                <div class="tagDiff rounded-pill p-2 mx-1 fw-bold">${recipeObject.difficulty}</div>
                <div class="tagCountry rounded-pill p-2 mx-1 fw-bold">
...
function main() {
  //attach click event
  btnTryAnotherRecipe.addEventListener("click", function (e) {
    let recipeId = getRandomRecipeIndex();
    FillRecipeInfo(recipeList[recipeId]);
    e.stopPropagation();
  });
  //start at recipe 0
  FillRecipeInfo(recipeList[0]);
}
